"""SQLite ledgers for casey-git-clone.

These ledgers are observer stores: StatiBaker may reference IDs/digests/hashes,
but must not ingest mutable candidate graphs as canonical SB state.
"""
...
def upsert_build(
    *,
    ...
    with _connect(db_path) as conn:
        ...
        conn.execute(
            "DELETE FROM casey_build_selection_refs WHERE build_id = ?",
            (record.build_id,),
        )
        for payload in selection_refs:
            ...
            conn.execute(
                """
                INSERT INTO casey_build_selection_refs(
                  build_id, path, fv_id
                ) VALUES (?,?,?)
                """,
                (record.build_id, str(payload.get("path") or ""), str(payload.get("fv_id") or "")),
            )
        conn.commit()
