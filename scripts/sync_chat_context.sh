...
PYTHONPATH="$ROOT_DIR/reverse-engineered-chatgpt"
...
if [[ ! -x "$PYTHON_BIN" ]]; then
  echo "Missing python venv at $PYTHON_BIN (expected root ITIR-suite .venv)" >&2
  exit 1
fi
...
while IFS='|' read -r _ raw_id raw_title raw_tail raw_notes _; do
  ...
  PYTHONPATH="$PYTHONPATH" "$PYTHON_BIN" "$LATEST_SCRIPT" \
    --conversation-id "$id" \
    --token "$TOKEN" \
    >> "$OUT_FILE"
  ...
  PYTHONPATH="$PYTHONPATH" "$PYTHON_BIN" "$VIEW_SCRIPT" \
    --conversation-id "$id" \
    --lines "1+" \
    --token "$TOKEN" \
    | tail -n "$tail_lines" \
