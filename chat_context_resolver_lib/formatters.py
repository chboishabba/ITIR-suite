...
def _print_db_plaintext(payload: dict) -> None:
...
def _print_web_plaintext(payload: dict) -> None:
    web = payload.get("web", {})
    print("web_command:")
...
def _print_error_plaintext(payload: dict) -> None:
...
def print_result(payload: dict, as_json: bool) -> None:
    ...
    if source == "db":
        _print_db_plaintext(payload)
        return

    if source == "web":
        _print_web_plaintext(payload)
        return

    _print_error_plaintext(payload)
