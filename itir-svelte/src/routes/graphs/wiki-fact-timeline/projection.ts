...
export type PropositionLinkRow = {
  link_id: string;
  event_id: string;
...
export type FactTimelineProjection = {
  facts: FactRow[];
  propositions: PropositionRow[];
  proposition_links: PropositionLinkRow[];
  diagnostics: {
    event_count: number;
    fact_row_source: 'native_fact_timeline' | 'nested_event_timeline_facts' | 'synthesized_from_steps';
...
function synthesizeFactsFromEvents(payload: WikiTimelineAooPayload): AooTimelineFact[] {
...
export function buildFactTimelineProjection(payload: WikiTimelineAooPayload): FactTimelineProjection {
