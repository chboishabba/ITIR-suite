...
- 2026-04-03 Wikidata/Nat live-follow receipt feedback:
  - landed:
    - `SensibLaw/src/ontology/wikidata_grounding_depth.py` now accepts
      bounded Nat live-follow result payloads on the grounding summary path
    - grounding evidence reports and priority surfaces now carry
      live-receipt state when a packet has fetched live evidence
  - effect:
    - a packet with fetched live receipts no longer looks like
      “search again”
    - the priority surface now marks that state as
      `live_receipts_ready_for_review`
    - the next bounded action becomes reviewing the fetched receipts on the
      packet, not issuing another follow
  - validation:
    - from repo root:
...
- 2026-04-03 Wikidata/Nat live-follow source-order fidelity:
  - landed:
    - `SensibLaw` Nat live-follow executor now resolves:
      - `named_query_link` from local sidecar and packet-coverage surfaces
      - `named_reference_url` from local packet, grounding, Cohort C, and
        Cohort B packet-input surfaces
    - Cohort B packet-input rows for:
      - `Q8646|P5991|4`
      - `Q11661|P5991|1`
      now pin concrete local `reference_url` values
  - effect:
    - split-heavy business-family rows now use `named_query_link` first
    - policy-risk preview rows now use `named_reference_url` first
    - reconciled non-business variance rows now also use
      `named_reference_url` first instead of falling back to
      revision-locked fetches
    - the full six-category Nat live-follow campaign is now operational on
      preferred local source order for its highest-value non-revision lanes
  - validation:
    - `SensibLaw`:
      - `../.venv/bin/python -m pytest -q tests/test_wikidata_nat_live_follow_executor.py tests/test_wikidata_cli.py -k 'live_follow_execute or live_follow_campaign'`
        -> `9 passed`
    - live runs:
      - `PYTHONPATH=SensibLaw .venv/bin/python -m SensibLaw.cli wikidata nat-live-follow-execute --input SensibLaw/tests/fixtures/wikidata/wikidata_nat_live_follow_campaign_20260403.json --category split_heavy_business_family --output /tmp/nat_live_follow_split_heavy_query_link_live.json`
        -> `chosen_source_class_counts={"named_query_link": 2}`
      - `PYTHONPATH=SensibLaw .venv/bin/python -m SensibLaw.cli wikidata nat-live-follow-execute --input SensibLaw/tests/fixtures/wikidata/wikidata_nat_live_follow_campaign_20260403.json --category reconciled_non_business_variance --output /tmp/nat_live_follow_reconciled_reference_url_live.json`
        -> `chosen_source_class_counts={"named_reference_url": 2}`

- 2026-04-03 Wikidata/Nat live-follow execution:
  - landed:
    - `SensibLaw` now has
      `src/ontology/wikidata_nat_live_follow_executor.py`
    - `sensiblaw wikidata nat-live-follow-execute` now executes the pinned
      Nat campaign manifest as a bounded live fetch run
    - the first two real rows were exercised live:
      - `hard_grounding_packet:1` -> revision-locked fetch for `Q10403939`
      - `split_heavy_business_family:1` -> query-link fallback to
        revision-locked fetch for `Q738421`
  - effect:
    - Nat is no longer only planning live follow; it can now run a bounded
      live fetch slice and capture revision-locked evidence on real rows
    - the executor preserves real fetch errors instead of masking them behind
      unsupported fallback source classes
  - validation:
    - `SensibLaw`:
      - `../.venv/bin/python -m pytest -q tests/test_wikidata_nat_live_follow_campaign.py tests/test_wikidata_nat_live_follow_campaign_plan.py tests/test_wikidata_nat_live_follow_executor.py tests/test_wikidata_cli.py -k 'live_follow_campaign or live_follow_execute'`
        -> `9 passed`
    - live runs:
      - `../.venv/bin/python -m cli wikidata nat-live-follow-execute --input tests/fixtures/wikidata/wikidata_nat_live_follow_campaign_20260403.json --category hard_grounding_packet --output /tmp/wikidata_nat_live_follow_result_hard_grounding.json`
        -> `status_counts={"fetched": 1}`
      - `../.venv/bin/python -m cli wikidata nat-live-follow-execute --input tests/fixtures/wikidata/wikidata_nat_live_follow_campaign_20260403.json --category split_heavy_business_family --limit 1 --output /tmp/wikidata_nat_live_follow_result_split_heavy_one.json`
        -> `status_counts={"fetched": 1}`

- 2026-04-03 Wikidata/Nat live-follow campaign selection:
  - landed:
    - pinned a first bounded multi-category Nat live-follow campaign in:
      - `SensibLaw/docs/planning/wikidata_nat_live_follow_campaign_20260403.md`
      - `SensibLaw/tests/fixtures/wikidata/wikidata_nat_live_follow_campaign_20260403.json`
    - the initial campaign spans:
  ...
  - effect:
    - the next Nat live-follow round now has a bounded target set with
      preferred source order, hop limits, and stop conditions
    - this keeps the lane local-first and review-first while finally moving
      beyond packet-only planning
  - validation:
    - `SensibLaw`:
      - `../.venv/bin/python -m pytest -q tests/test_wikidata_nat_live_follow_campaign.py`
        -> `2 passed`

- 2026-04-03 Wikidata/Nat live-follow execution plan:
  - landed:
    - `SensibLaw` now has
      `src/ontology/wikidata_nat_live_follow_campaign.py`
    - `sensiblaw wikidata nat-live-follow-campaign` now emits a bounded
      per-target execution plan from the pinned campaign manifest
  - effect:
    - the next Nat live run can operate from one explicit execution surface
      rather than re-deriving target rows and source order ad hoc
  - validation:
    - `SensibLaw`:
...
- 2026-04-03 Wikidata/Nat cohort-D workflow guidance:
  - landed:
    - `SensibLaw` cohort-D operator report now exposes:
      - `workflow_summary.stage`
      - `workflow_summary.recommended_view`
      - `workflow_summary.reason`
    - `SensibLaw` cohort-D review control index now also exposes the same
      read-only bounded packet guidance
  - effect:
    - Nat no longer forces bounded packet consumers to reconstruct the next
      operator move from queue sizes and blocker counts alone
    - the lane can now distinguish unresolved-reference cleanup from queued
      typing review at the packet/control level
  - validation:
    - `SensibLaw`:
...
- 2026-04-03 cross-lane uncertainty governance hardening:
  - landed:
    - root governance now explicitly asserts actionable uncertainty /
  ...
  - effect:
    - ranked follow queues, grounding-gap triage, and archive bounded-search
      signals are now pinned at the suite level rather than only inside their
      owning repos
  - validation:
    - root:
      - `.venv/bin/python -m pytest -q tests/test_cross_adopter_governance.py`
...
- 2026-04-03 legal-follow governance hardening:
  - landed:
    - root governance now explicitly asserts bounded legal-follow
      control-plane behavior for:
      - AU legal-follow operator view
      - GWB legal-follow operator view
  - effect:
    - the new legal-follow `follow.control.v1` surfaces are now pinned at the
      suite level and cannot silently regress back to graph-only summaries
  - validation:
    - root:
      - `.venv/bin/python -m pytest -q tests/test_cross_adopter_governance.py`
        -> `4 passed`

- 2026-04-03 AU legal-follow operator bridge:
  - landed:
    - `SensibLaw` AU legal-follow operator view now emits:
      - `follow.control.v1`
      - bounded queue items over derived legal follow targets
      - route-target and resolution summaries
    - `SensibLaw` AU authority-follow queue now also emits:
      - `priority_score`
      - `priority_rank`
      - `authority_yield`
    - shared AU fact-review bundles now also expose:
      - `workflow_summary.stage`
      - `workflow_summary.recommended_view`
      - `workflow_summary.reason`
    - the UK/British derived follow target is now surfaced as an actionable
      queue item rather than graph-only metadata
  - effect:
    - AU no longer stops at authority-follow queue plus legal-follow summary
    - it now has explicit legal-follow operator guidance for
      cross-jurisdiction uncertainty collapse
    - authority-follow is no longer just richly described; it is now ranked
      for bounded next-action triage
    - the bounded bundle itself can now tell downstream consumers what the
      next operator move should be without requiring the full workbench

- 2026-04-03 GWB review payload workflow guidance:
  - landed:
    - GWB public and broader review payloads now expose:
      - `workflow_summary.stage`
      - `workflow_summary.recommended_view`
      - `workflow_summary.reason`
  - effect:
    - bounded GWB review consumers no longer need to infer the next move from
      raw counts alone
    - the payload can now distinguish between legal-follow pressure,
      source-review pressure, and simple record-state capture
  - validation:
    - `SensibLaw`:
...
- 2026-04-03 Wikidata/Nat grounding priority surface:
  - landed:
    ...
    - incomplete packets expose:
      ...
      - recommended follow target `revision_locked_evidence`
      - `grounding_gap_class`
      - `recommended_follow_scope`
  - effect:
    - Nat no longer stops at aggregate grounding counts
    - it now has an explicit "what should we ground next?" surface under the
      uncertainty-collapse read
    - the lane can now distinguish revision-evidence gaps from broader
      packet-level grounding gaps
  - validation:
    - root:
      - `.venv/bin/python -m pytest -q SensibLaw/tests/test_wikidata_nat_grounding_depth.py`
        -> `15 passed`

- 2026-04-03 GWB/Brexit bounded follow bridge:
  - landed:
    - `SensibLaw` GWB legal-follow operator view now emits:
      - `follow.control.v1`
      - bounded queue items over followed legal sources
      - queue-level `priority_score`, `priority_rank`, and `authority_yield`
      - route-target summaries and resolution summaries
    - Brexit-relevant follows now route through explicit bounded targets such
      as `uk_legislation_follow` and `eur_lex_follow`
  - effect:
    - GWB/Brexit no longer stops at graph-only derived inspection
    - it now has the first control-plane bridge toward AU-style bounded
      uncertainty-collapse follow work
    - the queue is no longer flat; it now surfaces likely authority yield and
      the highest-priority bounded follow items first
  - validation:
    - `SensibLaw`:
...
- 2026-04-03 bounded-search planning + state-pressure checkpoint:
  - landed:
    - root join now emits `compatibility.uncertainty_surface` with:
      - dominant unresolved-pressure status
      - priority rank
      - bounded-search recommendation
    - root governance now asserts local-first bounded follow posture via
      explicit trigger/scope/stop and `derived_inspection` authority checks
    - `StatiBaker` drift now emits a read-only `context_dominance` signal
      with `dominant_thread_fraction` and `dominant_thread_id`
  - held:
    - `pyThunderbird` filtered index-search expansion
  - reason for hold:
    - compile passed, but the existing repo search tests still depend on
      unavailable `lodstorage`, so that slice is not yet promotion-grade
      beyond syntax
  - validation:
    - root:
...
- 2026-04-03 bounded-search control revision:
  - normalized adapters are now treated as sufficiently real infrastructure
  - next suite driver is bounded search and uncertainty collapse over
    existing proving-ground lanes and large corpora
  - AU, GWB, Brexit, Wikidata/Nat, and large archive/retrieval corpora now
    return to priority-driver status
  - key rule:
    - do not search by default
    - inspect current unresolved pressure first
    - prefer local context first
    - derive bounded search or follow only where expected authority yield and
      uncertainty-collapse value justify it
  - controlling note:
    - `docs/planning/suite_bounded_search_uncertainty_collapse_20260403.md`

- 2026-04-03 chat-export-structurer bounded archive-search pressure:
  - landed:
    - archive-search derived products now expose `search_bounds_status`
    - normalized products now also expose an `uncertainty_surface` with:
      - `local_archive_sufficient`
      - `recommended_next_bound`
  - effect:
    - large local archive search can now distinguish between already
      reviewable result sets and ones that need a narrower local query
  - validation:
    - repo root:
...
- 2026-04-03 dominant-disposition + ownership-audit round:
  - active drivers:
    - one more root join policy refinement
    - one more retrieval/archive family only if distinct ownership exists
    - TIRC chat-history ownership decision
  - landed:
    - root join now emits `dominant_disposition` above
      `named_incompatibilities`, `policy_summary`, and `policy_guidance`
  - correct no-ops:
    - `StatiBaker` hardening
    - `SensibLaw` authority-core
    - retrieval/archive expansion beyond current producer families
    - downstream governance extension
  - ownership decision:
    - do not prioritize further `tircorder-JOBBIE` chat-history
      normalization for now
    - treat it as an ownership-alignment/adopter surface because the
      underlying ChatGPT retrieval seam is more naturally owned by
...
- 2026-04-03 join severity-rollup refinement:
  - landed:
    - root join now emits `severity_summary` and `highest_severity` above the
      existing incompatibility policy surfaces
  - correct holds:
    - retrieval/archive family expansion
    - `tircorder-JOBBIE` chat-history deepening
  - validation:
    - root:
      - `.venv/bin/python -m pytest -q tests/test_suite_normalized_artifact_schema.py tests/test_cross_adopter_governance.py tests/test_normalized_artifact_join.py`
        -> `7 passed`

- 2026-04-03 TIRC chat-history + policy-guidance round:
  - active drivers:
    - next retrieval/archive family after `chat-export-structurer`
    - next join layer above `policy_summary`
  - landed:
    - `tircorder-JOBBIE` now has a producer-owned chat-history follow helper
      and a suite-normalized `derived_product` wrapper over bounded
      conversation discovery results
    - root join now emits per-code `policy_guidance` strings above named
      incompatibility records and disposition counts
    - root governance now checks the new TIRC chat-history family too
  - correct no-ops:
    - `SensibLaw` authority-core
    - further `StatiBaker` hardening
    - downstream adopter widening
  - validation:
    - root:
      - `.venv/bin/python -m pytest -q tests/test_suite_normalized_artifact_schema.py tests/test_cross_adopter_governance.py tests/test_normalized_artifact_join.py`
        -> `7 passed`
...
- 2026-04-03 archive-search + policy-summary round:
  - active drivers:
    - next retrieval/archive family after `reverse-engineered-chatgpt`
    - next join layer above named incompatibility records
  - landed:
    - `chat-export-structurer` now has a producer-owned archive-search follow
      helper and a suite-normalized `derived_product` wrapper over bounded
      FTS search results
    - root join now emits `policy_summary` over named incompatibility
      dispositions
    - root governance now checks the new archive-search family too
  - correct no-ops:
    - `SensibLaw` authority-core
    - further `StatiBaker` hardening
    - downstream adopter widening
...
- 2026-04-03 named incompatibility + next-family round:
  - active drivers:
    - root join/composition named incompatibility handling
    - next retrieval/archive family after `openrecall` and `pyThunderbird`
  - landed:
    - root join now emits `named_incompatibilities` records with explicit
      `code`, `severity`, `artifacts`, `reason`, and `disposition`
    - `reverse-engineered-chatgpt` now has a producer-owned bounded
      conversation-list follow helper and a suite-normalized
      `derived_product` wrapper for opt-in `--list` retrieval flows
    - root governance now imports and checks that new retrieval/follow family
  - correct no-ops:
    - `SensibLaw` authority-core
    - further `StatiBaker` hardening
    - downstream domain-adopter widening
  - validation:
    - root:
      - `.venv/bin/python -m pytest -q tests/test_suite_normalized_artifact_schema.py tests/test_cross_adopter_governance.py tests/test_normalized_artifact_join.py`
        -> `7 passed`
...
- 2026-04-02 five-lane refinement checkpoint:
  - requested lanes:
    - root join/composition refinement
    - stricter `StatiBaker` context-envelope validation
    - next retrieval/archive family after `pyThunderbird`
    - `SensibLaw` authority-core hardening only if a real gap appears
    - domain-adopter widening only after those are stable
  - results:
    - root join/composition landed a bounded compatibility refinement
    - `StatiBaker` landed stricter exported `context_envelope_ref` handling
    - `openrecall` landed a dedicated retrieval/follow helper that reuses the
      existing normalized search-follow contract
    - `SensibLaw` authority-core correctly stayed no-op
    - domain-adopter widening correctly stayed on hold
  - validation:
    - root:
      - `.venv/bin/python -m pytest -q tests/test_suite_normalized_artifact_schema.py tests/test_cross_adopter_governance.py tests/test_normalized_artifact_join.py`
        -> `7 passed`
...
- 2026-04-02 highest-leverage business-logic round:
  - lane set re-based to:
    - root join/composition semantics
    - `StatiBaker` state-core hardening
    - `SensibLaw` authority-core hardening only if a real gap appears
    - retrieval/follow convergence
    - provenance/context-envelope convergence
    - cross-family governance
    - older domain adopters only after that
  - landed:
    - root now has `normalized_artifact_join.py` plus
      `schemas/itir.normalized.artifact.join.v1.schema.json`
    - join helper now preserves artifact IDs, roles, authority classes,
      lineage, unresolved-pressure counts, and compatibility flags
    - `chat-export-structurer` now has `src/context_envelope.py` and package
      init so archive exports build canonical `context_envelope_ref`
      consistently
    - `StatiBaker` further tightened compiled-state lineage/context export
      semantics
    - `pyThunderbird` now emits bounded `retrieval.follow` artifacts and a
      normalized `derived_product` wrapper for `--mailid-like` searches
    - root governance now covers the new join seam and `pyThunderbird`
      retrieval-follow normalization
    - `SensibLaw` review/promotion lane correctly stayed no-op because no new
      authority-core gap justified churn
  - validation:
    - root:
      - `.venv/bin/python -m pytest -q tests/test_suite_normalized_artifact_schema.py tests/test_cross_adopter_governance.py tests/test_normalized_artifact_join.py`
        -> `7 passed`
...
- 2026-04-02 product-stack-first parallel round:
  - direction held:
    - do not let AU/GWB/Wikidata/Brexit and similar domain adopters drive the
      suite order
    - keep normalizing stack families in the owning repos
  - landed:
    - `tircorder-JOBBIE` now has
      `tircorder/normalized_source_sidecar.py` for producer-owned
      `source_artifact` sidecars aligned to the root normalized artifact
      contract
    - `notebooklm-py` now exposes one bounded non-authoritative
      `research follow` derived artifact posture for completed research output
    - `notebooklm-py` research follow now also has a producer-owned
      root normalized-artifact wrapper for read-only operator consumption
    - `reverse-engineered-chatgpt` download flow now also has a
      producer-owned root normalized conversation/source artifact for one
      explicit download target
    - `itir-svelte` normalized-artifacts view now reads one additional
      producer family through the same read-only inspection surface:
      explicit-path `chat-export-structurer` archive artifacts
    - `itir-svelte` normalized-artifacts view now also reads explicit-path
      `tircorder-JOBBIE` capture/source artifacts without reclassifying them
      as archive, state, or review output
    - `itir-svelte` normalized-artifacts view now also reads explicit-path
      `notebooklm-py` retrieval/research normalized artifacts without
      reclassifying them as archive, state, or promoted truth
    - `itir-svelte` normalized-artifacts view now also reads explicit-path
      `reverse-engineered-chatgpt` live-conversation/source artifacts without
      collapsing them into downstream archive or review roles
    - root now has `tests/test_cross_adopter_governance.py` to pin derived
      graphs, promotion gates, and unique derived-product roles across AU and
      GWB normalized products
    - `StatiBaker` compiled-state wrapper now accepts an explicit
      `context_envelope_ref` writer argument without changing the reducer
  - validation:
    - root:
    ...
    - `notebooklm-py`:
      ...
      - `../.venv/bin/python -m py_compile src/notebooklm/cli/helpers.py src/notebooklm/cli/research.py`
        -> passed
    - `tircorder-JOBBIE`:
      - manual normalized sidecar smoke passed
    - `itir-svelte`:
      - `npm run check` still fails only on the pre-existing
        `wiki-timeline-aoo-all/+page.svelte` errors

- 2026-04-02 suite-level `SL-reasoner` clarification:
  - intended boundary remains:
    - optional interpretive / hypothetical / non-authoritative layer over
      `SensibLaw`
  - actual implementation read:
    - most substantive deterministic reasoning still lives in `SensibLaw`
  - decision:
    - do not force migration into `SL-reasoner`
    - keep `SL-reasoner` low priority and scaffold-level for now
    - continue building in `SensibLaw`
    - revisit only if code complexity grows enough that extraction becomes a
      real simplification
  - landed bounded seam:
    - `SensibLaw` AU fact-review bundle now emits
      `semantic_context.reasoner_input_artifact`
    - `SL-reasoner` now validates that contract read-only and can emit
      explicit derived reasoning artifacts against it
    - no deterministic engine logic was extracted
  - controlling docs:
    - `SL-reasoner/README.md`
...
- 2026-04-02 moonshot compiler normalization reconsideration:
  ...
  - completion roadmap:
    ...
    - first operator-grade workflow layer
    - then remaining similarly prioritized workflow lanes
  - progress read:
  ...
  - completion condition:
    - AU, GWB, and Wikidata/Nat all fit one compiler-shaped contract
    - graph is derived rather than truth
    - reusable promotion gate is real
    - operator workflow is no longer materially behind the architecture
  - first implementation slice:
    - DONE: one tiny shared `compiler_contract` payload
    - landed fields:
      - `evidence_bundle`
      - `promoted_outcomes`
      - `derived_products`
    ...
    - normalize product shape only, not lane semantics
  - next promoted lane:
    - AU product normalization
    - DONE first bounded cut:
      - emit `semantic_context.compiler_contract` from the AU fact-review
        bundle so AU public handoff and AU review bundle share one compiler
        summary shape
    - result:
      - AU now has two normalized product surfaces under the shared contract:
        public handoff and fact-review bundle
  - next lane after AU:
    - GWB product normalization
    - DONE first bounded cut:
  ...
  - next lane after GWB:
    - reusable `promote | abstain | audit` gate
    - DONE first bounded cut:
      - landed `SensibLaw/src/policy/product_gate.py`
      - one shared gate record now sits above normalized products
      - first adopters:
        - AU public handoff
        ...
        - Wikidata migration pack
      - rule:
        - `promote` when promoted outcomes exist without open pressure
        - `abstain` when no promoted outcomes exist
        - `audit` when promoted outcomes coexist with review or abstain pressure
      - validation:
        - `28 passed`
  ...
  - next pinned lane:
    ...
    - DONE first bounded cut:
      - `SensibLaw/src/fact_intake/read_model.py` now emits one stable
        `workflow_summary` plus `semantic_context` in the workbench payload
      - `itir-svelte/src/routes/graphs/fact-review/+page.svelte` now surfaces
        the current workflow stage, recommended view, suggested fact focus, and
        compact pressure counts
      - validation:
        ...
        - repo-wide `npm run check` still fails on unrelated pre-existing
          `wiki-timeline-aoo-all` route errors
  - next pinned lane:
    - first bounded annotation / QA workbench slice over the existing
      fact-review/read-model/operator-view spine
  - legal moonshot expansion checkpoint:
    - broader legal moonshot remains:
      bounded evidence in -> promoted outcomes out -> derived legal
      packet/report/graph surfaces after that
    - AU should deepen first
    - next bounded AU legal slice:
      - derive one `legal_follow_graph` from existing authority receipts,
        legal refs, and citation candidates
    - anti-panopticon must stay explicit in README, legal planning, and
      red-team docs for this expansion
    - controlling notes:
    ...
    - current bounded dual slice:
      - deepen AU supporting-legislation / cited-instrument attachment
        provenance where it materially improves inspectability
      - surface the derived `legal_follow_graph` in the fact-review workbench
        as a read-only inspection surface
      - keep graph posture unchanged: derived-only, challengeable, optional,
        and non-authoritative
    - landed result:
      - `legal_follow_graph.summary` now exposes supporting receipt counts and
        supporting authority-kind counts
      - AU authority-follow payloads now also expose bounded
        jurisdiction-hint and instrument-kind counts for attached legal refs
      - AU authority-follow payloads now also expose bounded ref-kind counts
      - AU authority-follow payloads now also expose bounded citation-court
        and citation-year counts
      ...
      - supporting-legislation nodes now keep `supporting_legislation_roles` and counts for enabling/constraining/procedural axes
      - AU legal-follow graph now also reports bounded reference-kind,
        reference-class, ref-kind, and edge-kind distributions
      - AU legal-follow graph now also reports bounded citation-court and
        citation-year distributions
      - fact-review workbench now surfaces one read-only legal-follow graph
        inspection pane with summary, authority/receipt, ref/citation, and
        typed-link sections
      - that pane can also render bounded distribution grids when a derived
        legal-follow graph exposes them
      - focused AU/legal/compiler gate: `21 passed`
      - frontend check still only blocked by the pre-existing
        `wiki-timeline-aoo-all` route errors
    - GWB parity result:
      - `SensibLaw/src/policy/gwb_legal_follow_graph.py` now provides one
        derived GWB legal-linkage graph helper
      - `gwb_public_review` and `gwb_broader_review` now emit
        `legal_follow_graph`
      - their summary markdown now exposes a bounded "Derived Legal-Linkage
        Graph" section
      - that summary now also exposes bounded source-kind, source-family,
        linkage-kind, review-status, and support-kind distributions so the
        graph is more inspectable without inventing a separate operator UI
      - GWB review summaries now also expose bounded graph-inspection and
        sample-typed-link sections for read-only graph inspection
      - GWB review payloads now also expose one bounded JSON
        `operator_views.legal_follow_graph` surface with summary,
        highlight-node, and sample-edge inspection data
      - the fact-review workbench now renders that same bounded
      ...
      - GWB legal-linkage graphs now add bounded followed-source nodes when
        source-review receipts already carry HTTP links
      - the existing fact-review legal-follow pane is typed to render those
        bounded graph distributions when present, without changing the
        derived-only posture
      - GWB review contracts now list `legal_linkage_graph` as an explicit
        derived product
      - focused AU+GWB/compiler gate: `25 passed`
    - normalization read:
      - the legal moonshot is now normal program state, not a speculative side lane
      - AU remains the semantically richer legal lane
      - GWB now has artifact-level, JSON-level, and workbench-level legal-follow
        surfaces under the same compiler-shaped rule
      - next bounded AU cross-jurisdiction step is one explicit AU ->
        UK/British follow hop when current evidence already points there
      - that hop stays provenance-backed, derived-only, and review-first
      - next bounded GWB cohort candidates include previous US presidents and
        UK Brexit-era politicians where legal consequences are especially
        legible in public source geometry
      - Brexit should now be treated as a named bounded legal-union proving
        ground, not only as a cohort example
      - landed follow-through:
        - AU legal-follow now emits one derived UK/British follow target when
          receipt/ref/citation evidence already points there
        - GWB legal-linkage now emits bounded cite-class and Brexit-related
          follow counts from existing followed-source URLs
        - GWB legal-linkage now also seeds followed-source receipts from the
          foundation-source catalog when a review row already names a known
          UK/EU legal source
      - progress/full-flow read:
        ...
        - end-state operational flow:
          evidence intake -> canonicalization -> typed extraction -> bounded
          follow planning/execution -> promote/abstain/hold -> derived graph
          construction -> graph union -> commonality/disjointness/basin/
          contradiction analysis -> operator inspection -> bounded products
          under anti-panopticon governance
      - root correction:
        - this is only the legal proving-ground read, not the full suite read
        - the broader suite moonshot is the original smart-journal/compiler
          arc:
          capture -> archive -> canonicalize -> compile state -> review /
          promote -> derive -> union -> inspect
      - suite-wide invariants are now explicit at the root:
        provenance, expansion, context, authority, replay, derived-only,
        append-only state, anti-panopticon

- 2026-04-02 suite P0 normalized-concepts reset:
  - source: user correction that the framing was still too `SensibLaw`-specific
    and should generalize across the submodules back to the original
    smart-journal arc
  - main decision:
    - current plans are stepping stones, not the destination
    - the new P0 suite moonshot is to converge the suite on shared normalized
      concepts:
      - source artifact
      - provenance anchor
      - context envelope
  ...
  - control implications:
    - capture/transcription projects are source-artifact and provenance-anchor
      producers
    - archive/acquisition/retrieval projects are canonical-identity and
      context-envelope producers
    - state projects are observed-signal -> compiled-state reducers
    - legal/reasoner projects are review/promotion/derived-product adopters
    - operator/integration projects are inspection surfaces
  - added suite-level hard control:
    - no follow expansion without an explicit bounded trigger
  - controlling note:
    - `docs/planning/suite_p0_moonshot_normalized_concepts_20260402.md`
  - root sync:
    - `README.md`, `TODO.md`, and `CHANGELOG.md` now treat the existing
      plans as stepping stones under the new P0 suite moonshot
  - executable followthrough:
    - added
      `docs/planning/suite_p0_completion_roadmap_20260402.md`
    - added root schema
      `schemas/itir.normalized.artifact.v1.schema.json`
    - added minimal fixture
      `examples/itir.normalized_artifact.minimal.json`
    - added focused contract gate
      `tests/test_suite_normalized_artifact_schema.py`
    - first machine-readable guarantees:
      - promoted records require promoted-truth authority and receipt linkage
      - derived products remain explicitly derived
    ...
    - validation:
      - `.venv/bin/python -m pytest -q tests/test_suite_normalized_artifact_schema.py`
        -> `3 passed`
      - minimal fixture validated against the schema
    - first bounded adopter landed:
      - `SensibLaw` AU fact-review bundle now emits
        `semantic_context.suite_normalized_artifact`
      - that payload validates against the root
        `itir.normalized.artifact.v1` schema
      - it is derived from the existing `compiler_contract` and
        `promotion_gate` surfaces rather than a second reducer
      - focused adopter gate:
        - from `SensibLaw`:
          `../.venv/bin/python -m pytest -q tests/test_au_fact_review_bundle.py`
          -> `6 passed`
    - control correction:
      - do not prioritize older domain-specific proving grounds just because
        they already have active lanes
      - AU/GWB/Wikidata/Nat/Brexit/related lanes remain adopters and proving
        grounds, but the suite order is now product-stack-first in the owning
        repos
    - first bounded state adopter landed:
      ...
      - that payload is a `compiled_state` artifact aligned to the root
        `itir.normalized.artifact.v1` schema
      - it is built from the existing `state.json` output rather than a
        second reducer
      - validation:
        - from repo root:
          `.venv/bin/python -m pytest -q tests/test_suite_normalized_artifact_schema.py`
        ...
        - from `StatiBaker`:
          `../.venv/bin/python -m pytest -q tests/test_suite_normalized_artifact.py tests/test_bundle.py`
          -> `4 passed`
    - first bounded operator adopter landed:
      - `itir-svelte` now exposes `/graphs/normalized-artifacts`
      - that route reads one current `SensibLaw` normalized artifact and one
        current `StatiBaker` normalized artifact directly from the producing
        repos
      - it stays read-only and avoids local reinterpretation between compiled
        state and review output
      - frontend validation:
        - `npm run check`
        - only remaining failures are the unrelated pre-existing
          `wiki-timeline-aoo-all` route errors
    - first bounded capture/archive adopter landed:
...
- 2026-03-29 AAO runtime/route roadmap checkpoint:
  - source: current working turn
  - main decision:
    - the AAO timeline routing helpers now live under `itir-svelte/src/lib/server/wiki_timeline/`
    - `itir-svelte/src/lib/server/wikiTimelineAoo.ts` is now a thin adapter sharing those helpers
    - the `wiki-timeline-aoo-all` route consumes the shared `filters`, `graph`, and `selection` helpers, with the controls and context panels extracted into `ControlsPanel.svelte` and `ContextPanel.svelte`
    - the remaining follow-up is the route-owned graph/evidence assembly residue, not the controls/context shell
  - documentation artifacts updated:
    ...
    - `CHANGELOG.md`
  - validation:
    - `cd itir-svelte && node --test tests/graph_ui_regressions.test.js`
...
- 2026-03-29 notebooklm-pack boundary check:
  ...
  - docs path status:
    - `/home/c/Documents/code/kant-zk-pastebin/DOCS.md` exists as a symlink,
      but its target under `/home/mdupont/DOCS/...` is unavailable in this
      environment
  - main decision:
    - `../notebooklm-pack` is a bounded Rust utility for repo-text collection
      and NotebookLM source clustering only
    - it should not be treated as evidence for `ZOS <-> SL` semantics, `JMD`
      push/pull, admissibility, resonance/manifold logic, or proof/receipt
      boundaries
  - documentation artifacts added/updated:
...
- 2026-03-29 notebooklm-pack integration seam:
  ...
  - main decision:
    - the sibling pack is useful as an upstream source-pack producer for
      NotebookLM Channel A source ingress
    - the correct container order is:
    ...
    - if implemented later, the first bounded seam should be a tiny wrapper or
      manifest-normalizer, not a semantic bridge
...
- 2026-03-29 notebooklm-pack dry-run wrapper:
  - source: current working turn
  - main decision:
    - implement the smallest honest seam as a dry-run-first wrapper rather than
      jumping straight to auth-backed live NotebookLM mutation
  - implementation:
    - added `scripts/notebooklm_pack_ingest.py`
    - wrapper supports:
      - optional `notebooklm-pack` execution
      - manifest normalization
      - source file hashing
      - deterministic `notebooklm` command planning
      - optional live execution behind `--execute`
  - validation:
    - added `tests/test_notebooklm_pack_ingest.py`
    - local test result:
      `.venv/bin/python -m pytest -q tests/test_notebooklm_pack_ingest.py`
      -> `4 passed`
  - live followthrough:
    - local authenticated NotebookLM execution now succeeds
    - real wrapper mismatches were found and fixed:
      - nested `notebook` JSON shape from `create --json`
      - nested `source` JSON shape from `source add --json`
      - local CLI version does not support `--interval` on `source wait`
      - local CLI discovery needed to prefer the repo `.venv` binary
    - persistent validation notebook kept:
      - title: `ITIR notebooklm-pack integration`
      - id: `ad2bbd9a-2c9c-47ee-a607-f2b735999d99`
      - url:
        `https://notebooklm.google.com/notebook/ad2bbd9a-2c9c-47ee-a607-f2b735999d99`
    - seeded linkage artifact:
      `/home/c/Documents/code/ITIR-suite/.cache_local/notebooklm_pack_runs/20260329_persistent_integration_seed/result.json`
    - NotebookLM produced a coherent two-source summary for the kept notebook,
      which is a practical success signal for the pack->ingest loop
  - next gap:
    - downstream StatiBaker linkage enrichment remains separate follow-up work
    - the next JMD question is no longer NotebookLM liveness; it is seam
      object disambiguation and receipt-vs-observer classification

- 2026-03-29 JMD NotebookLM seam disambiguation:
  - source: current working turn
  - main decision:
    - the new live NotebookLM loop does not make NotebookLM metadata into JMD
      receipts by itself
    - the right next bounded object is `NotebookLMExternalLink.v1`
  - minimal fields:
    - `pack_run_id`
    - `source_file_hash`
    - `notebook_id`
    - `notebook_source_id`
    - `artifact_ids`
    - bounded titles and timestamps
  - classification rule:
    - titles, counts, statuses, and NotebookLM-generated summaries remain
      observer metadata
    - hashes, manifest linkage, and stable external ids are receipt
      candidates only if a sharper JMD contract later adopts them
  - artifact:
...
- 2026-03-28 bounded climate text producer into the Wikidata `Phi` bridge:
  - source: current working turn
  - main decision:
    - the first real text-side producer for the `P5991 -> P14143` lane should
      stay narrow and revision-locked rather than pretending to be general text
      understanding
    - the producer emits `sl.observation_claim.contract.v1` rows from explicit
      year/value climate lines, then feeds those rows into the existing
      migration-pack bridge as additive pressure only
  - implementation:
    - schema:
      `SensibLaw/schemas/sl.wikidata.climate_text_source.v1.schema.yaml`
    - runtime helpers:
      `SensibLaw/src/ontology/wikidata.py`
    - materializer hook:
      `SensibLaw/scripts/materialize_wikidata_migration_pack.py`
  - validation:
    - `/home/c/Documents/code/ITIR-suite/.venv/bin/python -m pytest -q SensibLaw/tests/test_wikidata_projection.py SensibLaw/tests/test_materialize_wikidata_migration_pack.py SensibLaw/tests/test_wikidata_cli.py SensibLaw/tests/test_observation_claim_wikidata_projection.py`
  - target-selection result:
    - `HSBC` / `Q190464` is not currently a valid lane target because it
      currently returns no live `P5991` statements
    - the first real artifact hunt should therefore pivot to already-pinned
      entities with live `P5991`, especially `Q10422059`
      (`Atrium Ljungberg`) and `Q10403939` (`Akademiska Hus`)
  - first non-fixture artifact now exists:
    ...
    built from official Akademiska Hus annual report excerpts for 2018, 2019,
    and 2020
  - first real bridge result:
    ...
    candidates
  - interpretation:
    this is useful because it shows the bridge refusing to equate older
    scope-1 totals with the current 2023 multi-scope structured bundle, while
    no longer overstating that mismatch as hard contradiction
  - next gap:
...
- 2026-03-28 ITIR / SensibLaw identity-trust alignment refinement:
  - source: current working turn
...
- 2026-03-28 refreshed online thread sync:
  ...
  - main decision:
    ...
    but the next maturity gap is operational readiness rather than more
    ontology
  - newly-sharpened gaps:
    - missing service-level definitions
    - missing incident vs problem distinction
    - missing measurable success criteria
    - system-boundary / handoff views still too implicit
  - implementation note:
    resolving this thread exposed a refactor regression in
    `scripts/chat_context_resolver.py`; stale `_truncate_text` references were
    fixed so DB-backed UUID resolution works again

- 2026-03-28 ITIR / SensibLaw standard service application model:
  - source: current working turn
  - documentation artifact added:
...
- 2026-03-28 ITIR / SensibLaw everyday mode:
  - source: current working turn
  - documentation artifact added:
...
- 2026-03-28 ITIR / SensibLaw case-type libraries + KPI model:
  - source: current working turn
  - documentation artifact added:
...
- 2026-03-28 ITIR / SensibLaw diagrams + mode-switching / UI / templates:
  - source: current working turn
  - documentation artifacts added:
  ...
  - product decision:
    ...
    views
  - stricter mode note:
    strict mode is now explicitly tied to actor/time/fallback obligations,
...
- 2026-03-28 ITIR / SensibLaw production schema / dashboard / deployment pack:
  - source: current working turn
  - documentation artifact added:
...
- 2026-03-28 largest-file refactor / normalization audit:
  ...
  - main decision:
    ...
    - the first five Priority 1 targets now satisfy that requirement and are
      ready for actual triage/execution sequencing
...
- 2026-03-28 first `chat_context_resolver` extraction slice:
  - source: current working turn
  - implementation:
    - added helper package:
      `chat_context_resolver_lib/`
    - added transcript helpers in:
      `chat_context_resolver_lib/transcript.py`
    - added transcript-analysis helpers in:
  ...
  - main decision:
    - the first bounded implementation slice for the refactor lane is the
      transcript/analysis subsystem, not DB lookup or live-provider fallback
    - the helper package uses `chat_context_resolver_lib/` because the repo
      already contains the file `scripts/chat_context_resolver.py`
    - live-provider and DB lookup remain explicit pending seams for later
      slices
  - validation:
    - `pytest -q tests/test_chat_context_resolver_analysis.py`
...
- 2026-03-29 `Phi` v2 boundary note carried in the shipped `v1` payload:
  - source: current working turn
  - implementation:
    - added `versioning_note` to:
      `SensibLaw/src/cross_system_phi.py`
      `SensibLaw/schemas/sl.cross_system_phi.contract.v1.schema.yaml`
      `SensibLaw/examples/cross_system_phi_minimal.json`
  - main decision:
    - the shipped `v1` transport payload now states its own boundary
      explicitly: `exact|partial|incompatible|undefined` remains the current
      grammar, while future `Phi v2` semantics are reserved for a separate
      schema that can split relation kinds, scores, and richer latent graph
      structure
    - this keeps the next semantic layer visible without turning it into a
      hidden truth tier inside the current contract
  - validation:
    - `/home/c/Documents/code/ITIR-suite/.venv/bin/python -m pytest -q SensibLaw/tests/test_cross_system_phi_schema.py SensibLaw/tests/test_cross_system_phi_prototype.py`
...
- 2026-03-28 Publish-layer findings before Rust template exploration:
  - source: current working turn
  - planning note:
    `docs/planning/publish_layer_findings_20260328.md`
  - main decision:
    - the uncertainty is not whether the suite has a publication concern; it is
      where to freeze the publish-layer boundary and ownership
    - current best-fit read remains:
      ...
      consumer
    - the first publish-layer prototype should stay narrow:
      logical artifact input -> digest and sink refs -> receipt output
    - query routing, JMD bridge semantics, and promotion/governance stay
      outside that first publisher slice
    - `../rust-nix-template` is useful as scaffolding for a Rust-facing
      transform/publisher seam, but it does not by itself settle the shared
      sink contract
    - the current template `src/main.rs` is scaffold-quality and not a clean
      implementation baseline
  - followthrough:
    - recorded in `README.md` and `TODO.md`
    - no code changes were made in this pass

- 2026-03-28 Robust context fetch for ZOS / SL / Zelph ordering:
  - source: db after direct online UUID pull into canonical archive
  - title:
  ...
  - main decision:
    - the fetched thread sharpens the intended role split as:
      `ZOS -> SL -> Zelph`
    - `ZOS` is the dynamic candidate-state / evolving ontology layer
    - `SL` is the deterministic extraction and promotion layer
    - `Zelph` is the downstream fact-graph / query / reasoning layer
    - the intended next-step order from the thread is:
      define the `ZOS <-> SL` contract, build a minimal ZOS engine in Python,
      then map ZOS outputs into a Zelph input layer
    - this means the publish-layer question is still real, but it sits
      downstream of the `ZOS -> SL -> Zelph` boundary rather than replacing it
    - refreshed online pull on 2026-03-28 inserted new turns that make the
      disambiguation stricter:
      ZOS must not replace/supplant SL; it should supplement SL by consuming
      structured promoted facts and proposing overlays or candidate facts back
      through the SL boundary
    - the refreshed thread also rules out naive token-frequency ZOS:
      first ZOS prototypes should operate on structured fact inputs
      `(predicate, arguments, qualifiers)` rather than raw text co-occurrence
    - practical enforcement rule:
      if ZOS overrides SL truth state, that is a design error; if it organizes
      SL facts and routes proposals back through SL promotion, that is
      acceptable layering
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-28 ZOS vs fuzzymodo / Casey / StatiBaker role comparison:
  - source: current working turn
  - planning note:
    `docs/planning/zos_vs_fuzzymodo_casey_statiBaker_20260328.md`
  - main decision:
    - `ZOS` is not best compared equally to all three existing surfaces
    - it is closest to `fuzzymodo` as a supplemental reasoning/organization
      layer over already-exposed state
    - it is not a good role match for `casey-git-clone`, which owns mutable
      possibility/workspace/build state
    - it is not a good role match for `StatiBaker`, which owns observer-only
      memory, receipts, and timeline/alignment state
    - the real overlap risk is `ZOS` duplicating or fuzzily re-describing work
      that may already fit inside `fuzzymodo`
    - current recommended posture:
      if `ZOS` becomes concrete, either narrow it into a semantic sub-lane of
      `fuzzymodo`, or keep it separate with explicit prohibitions against truth,
      mutable workspace, and observer-memory ownership
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-28 Publisher/puller contract clarification for Zelph consumers:
  - source: current working turn
  - planning note:
    `docs/planning/publisher_puller_contract_for_zelph_consumers_20260328.md`
  - main decision:
    - `Zelph` should be described as the read/query consumer side, not as the
      publish-layer authority
    - publisher/puller flow should stay explicit as:
      logical artifact -> shard ids -> sink refs -> receipt
    - Zelph consumer flow should stay explicit as:
      selector -> logical shard ids -> fetch -> Zelph load/query
    - the useful abstraction for `ITIR` is therefore:
      upstream truth/promotion, downstream Zelph query consumption, and a
      separate publish/pull artifact-serving layer between them
    - this sharpens the earlier shard-matrix and publish-layer notes without
      changing their role-fit conclusions
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 JMD push/pull blocker separation:
  - source: current working turn
  - planning note:
    `docs/planning/jmd_push_pull_surfaces_and_blockers_20260329.md`
  - main decision:
    - the refreshed JMD thread clarifies semantic/governance boundaries
      (`ZOS` supplements `SL`; `Zelph` stays downstream) and also adds a
      stronger proof-first API framing:
      JMD describes service proof in terms of Nix flake, git history, perf
      profile, and resource use, and explicitly says `zkperf + erdfa` is "the
      API"
    - one provisional contract inference is now reasonable:
      `artifact + erdfa payload + zkperf receipt`
      but this remains inference rather than declared JMD spec
    - even with that stronger inference, the thread does not by itself define
      a stable JMD push/pull host contract
    - `../rust-nix-template` clarifies a plausible local Rust/Nix home for a
      publisher/puller seam, but it does not pin remote JMD endpoint, receipt,
      or replay semantics
    - the still-missing blocker for moving beyond the current
      `kant-zk-pastebin` / `erdfa-publish-rs` reference pair remains external
      infra certainty:
      stable browse/raw behavior, replay/cache semantics, and publication
      acknowledgement shape
    - `ITIR` should therefore keep semantic boundary findings, local scaffold
      status, and external infra uncertainty as three separate planning inputs
      rather than treating any one of them as a full replacement for the
      others
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 Zelph / ERDFA / HF / IPFS example-flow clarification:
  - source: current working turn
  - planning note:
    `docs/planning/zelph_erdfa_hf_ipfs_example_flow_20260329.md`
  - main decision:
    - the concrete current repo reading should be explained as:
      publisher emits logical shards and sink refs, then `Zelph` resolves
      selectors to logical shard ids before choosing an HF/IPFS object ref
    - `erdfa`/Kant-style packaging is the publish-side reference for shard
      identity, digests, manifests, and sink refs
    - HF and IPFS are alternate object locations for the same logical shard,
      not competing semantic models
    - `Zelph` must route by logical selector -> logical shard id first, not by
      HF path structure or IPFS CID structure
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 `n00b` corroborating-surface check:
  - source: current working turn
  - planning note:
    `docs/planning/n00b_corroborating_surfaces_20260329.md`
  - main decision:
    - `n00b/` is useful as corroborating evidence for a proof-first /
      HF-hosted / Nix-backed direction, but not as a new authoritative ITIR
      contract source
    - `n00b/.gitmodules` declares these relevant source repos:
    ...
    - this strengthens ecosystem evidence for HF-hosted shards and
      `erdfa-publish` on the publish side, but does not reduce the current
      uncertainty around Zelph routing contracts or JMD infra semantics
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass
...
- 2026-03-29 `erdfa-publish-rs` vs shared-shard-contract comparison:
  - source: current working turn
  - planning note:
    `docs/planning/erdfa_publish_rs_vs_shared_shard_contract_20260329.md`
  - main decision:
    - local `/home/c/Documents/code/erdfa-publish-rs` materially strengthens
      the publish-side evidence:
      it already has `Shard`, `ShardSet`, deterministic content-derived `cid`,
      DA51 CBOR serialization, tar emission, and IPFS/paste helpers
    - it aligns with the ITIR contract on publish/package primitives, but it
      does not yet carry the richer fields ITIR wants for Zelph-facing shared
      artifacts:
      artifact revision/provenance, routing keys, explicit multi-sink
      `objectRefs[]`, and first-class per-shard `section` / `logicalKind` /
      `encoding` / `sizeBytes`
    - therefore the correct posture is:
      keep `erdfa-publish-rs` as the publish-side substrate and keep the shared
      ITIR contract as the thin richer layer above it, rather than collapsing
      the shared contract into the current `ShardSet` model
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 Robust context fetch for message-envelope direction:
  - source: db after direct online UUID pull into canonical archive
  - title:
  ...
  - planning note:
    `docs/planning/canonical_zkp_sl_da51_message_schema_findings_20260329.md`
  - main decision:
    - the fetched thread is useful as a future envelope-layer reference:
      content-addressed message identity, JSON/CBOR dual projection,
      provenance/parent DAG links, and optional proofs/signatures
    - this sharpens the distinction between:
      shard/artifact contracts ("what artifact is this?") and
      message/envelope contracts ("what signed/provenanced transmission is
      carrying it?")
    - it should not yet be treated as a normative ITIR wire standard because
      hashing rules, exact CBOR layout, signature suite, proof attachment
      conventions, and transport semantics are still unpinned
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 Robust context fetch for formal toolchain watchlist:
  - source: db after direct online UUID pull into canonical archive
  - title:
    `2025‑26 formal toolchain watchlist`
  - online UUID:
    `69c3f3fa-b1d0-83a0-9954-27a65a228a03`
  - canonical thread ID:
    `4713552f782e95ec70a2f93813bcca4b57156f09`
  - main read:
    - useful watchlist context around DSLean / Lean solver interop and adjacent
      formal-tooling developments
    - not sharp enough to justify a new repo-facing contract or planning note
      in this pass
  - followthrough:
    - retained as follow-up context only; no dedicated doc update in this pass

- 2026-03-29 Robust context refresh for JMD HF containerization and spectral retrieval:
  - source: db after repeated direct online UUID pulls into canonical archive
  - title:
    `JMD FORMAL EXPLAIN - Meme System Explanation`
  - online UUID:
    `69c4a9b1-d014-83a0-8bb0-873e4eaa4098`
  - canonical thread ID:
    `c6e383233d0d7c4efde671be1432c825054cb222`
  - planning note:
    `docs/planning/jmd_hf_container_and_spectral_retrieval_findings_20260329.md`
  - main decision:
    - the thread now materially sharpens three repo-facing points:
      the logical artifact contract is more mature than "missing API", HF
      file-count/rate pressure requires a container/index design read, and
      spectral retrieval should be treated as a post-selector ranking layer
      rather than a replacement for selector/shard routing
    - useful batching direction from the thread:
      ...
      optional `zstd`
    - useful retrieval constraint from the later turns:
      any eigenvector/spectral layer must use structured SL facts or zkperf
      trace structure rather than raw tokens or generic embeddings
    - latest navigator/manifold turns also sharpen a domain-validity rule:
      symmetry/meaning should be treated as valid within a manifold; projection
      collapse is a real failure mode
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 HF container/index contract pass:
  - source: current working turn
  - planning note:
    `docs/planning/hf_container_index_contract_20260329.md`
  - main decision:
    - HF file-count/rate pressure should now be modeled as a distinct batching
      layer, not as a reason to blur logical shards into HF object layout
    - correct ordering is:
      logical shards -> container membership -> uploaded HF objects ->
      published index
    - selector-first retrieval remains mandatory even under batching:
      selector -> logical shard ids -> container index -> HF object ->
      member payload -> local load/query
    - current best-fit physical direction is:
      `tar` outer format, `CBOR` payload members, `JSON` control/meta, optional
      `zstd`
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 HF container/index fixture v1 pass:
  - source: current working turn
  - planning note:
    `docs/planning/hf_container_index_fixture_v1_20260329.md`
  - fixture:
    `docs/planning/jmd_fixtures/hf_container_index_v1.example.json`
  - main decision:
    - the first tiny batching fixture now exists and keeps three levels
      separate:
      artifact identity, container metadata, and member metadata
    - each member entry now explicitly repeats:
      `shardId`, `contentDigest`, `memberPath`, and `sizeBytes`
    - HF object path remains confined to container metadata and does not become
      shard identity
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 `erdfa-publish-rs` manifest-promotion v1 pass:
  - source: current working turn
  - planning note:
    `docs/planning/erdfa_publish_rs_manifest_promotion_v1_20260329.md`
  - main decision:
    - the second ranked step is now pinned as a manifest-layer promotion, not
      a shard-payload rewrite
    - keep existing `Shard`, `cid`, and DA51/CBOR packaging discipline intact
    - add only the smallest richer manifest metadata:
      artifact identity/provenance plus per-shard `logicalKind`,
      `encoding`, `sizeBytes`, and optional `objectRefs[]` / `routingKeys[]`
    - keep routing/runtime behavior out of this first promotion slice
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 `erdfa-publish-rs` promoted-manifest fixture implementation:
  - source: current working turn
  - fixture:
    `docs/planning/jmd_fixtures/erdfa_manifest_promotion_v1.example.json`
  - main decision:
    - the second ranked design step now has a concrete fixture showing
      artifact-level identity/provenance plus enriched shard refs with
      `logicalKind`, `encoding`, `sizeBytes`, optional `objectRefs[]`, and
      `routingKeys[]`
    - this fixture keeps existing shard id / `cid` semantics intact while
      enriching only the manifest/catalog layer
  - validation:
    ...
    - combined targeted run:
      ...
      passed
  - followthrough:
    - recorded in `TODO.md` and `CHANGELOG.md`

- 2026-03-29 HF container rehearsal harness implementation:
  - source: current working turn
  - main decision:
    - the first ranked implementation-facing rehearsal slice is now real in
      `itir_jmd_bridge`
    - new module:
      `itir_jmd_bridge/hf_rehearsal.py`
      loads the pinned HF container fixture and resolves `shardId` to
      container/member metadata
    - new CLI command:
      `python -m itir_jmd_bridge rehearse-hf-container --fixture ... --shard-id ...`
    - targeted regression coverage added in:
      `tests/test_hf_container_rehearsal.py`
  - validation:
    - `pytest -q tests/test_hf_container_rehearsal.py` passed
  - followthrough:
    - recorded in `TODO.md` and `CHANGELOG.md`

- 2026-03-29 Selector-to-container rehearsal composition:
  - source: current working turn
  - main decision:
    - the local rehearsal path now composes the promoted-manifest fixture with
      the HF container fixture so the suite can resolve:
      selector -> logical shard -> container member
    - new functions in `itir_jmd_bridge/hf_rehearsal.py`:
      `load_erdfa_manifest_fixture`, `resolve_selector_to_shard`, and
      `resolve_selector_to_container_member`
    - new CLI command:
      `python -m itir_jmd_bridge rehearse-selector-fetch --manifest-fixture ... --container-fixture ... --selector ...`
  - validation:
...
- 2026-03-29 Local tar extraction added to rehearsal path:
  - source: current working turn
  - main decision:
    - the rehearsal path now goes one step further and can extract the resolved
      member payload from a local tar archive
    - new helper functions in `itir_jmd_bridge/hf_rehearsal.py`:
      `extract_tar_member_bytes` and
      `resolve_selector_to_local_member_payload`
    - new CLI commands in `itir_jmd_bridge/cli.py`:
      `rehearse-local-tar-extract` and
      `rehearse-selector-local-tar-fetch`
    - a real tar was generated from `/home/c/Documents/code/erdfa-publish-rs`
      via `cargo run --example demo`, producing `/tmp/erdfa-demo.tar`, while
      deterministic tests use a tiny temp tar matching the fixture member paths
  - validation:
    - `pytest -q tests/test_hf_container_rehearsal.py tests/test_erdfa_manifest_promotion_fixture.py`
      passed
  - followthrough:
...
- 2026-03-29 `erdfa-publish-rs` promoted-manifest code patch:
  - source: current working turn
  - repo:
    `/home/c/Documents/code/erdfa-publish-rs`
  - main decision:
    - the additive manifest-layer patch is now real in the upstream publish
      crate rather than only in ITIR fixtures
    - `src/lib.rs` now adds `BuildProvenance`, `ObjectRef`,
      `PromotedShardRef`, and `PromotedShardSet` while preserving existing
      `Shard` and `ShardSet` APIs
    - `Shard` now exposes helper conversions for both the thin manifest ref and
      the promoted ref, so the richer catalog layer can be adopted without a
      shard-payload rewrite
    - `/home/c/Documents/code/erdfa-publish-rs/README.md` now documents the
      promoted-manifest usage slice
  - validation:
    - `cargo test -q` passed in `/home/c/Documents/code/erdfa-publish-rs`
  - followthrough:
    - recorded in `TODO.md` and `CHANGELOG.md`

- 2026-03-29 RG toy completion thread:
  - source: db after async pull of online UUID `69c89099-c7f4-839c-a9ac-93246ca708e7` (title: `RG toy completion steps`), canonical thread ID `fb062abe619bc64e9c50719311bb3a0374a38b6c`
  - main finding:
    remaining work is proof/content, not shell: need sharper coarse-agreement relation, real coarse-graining operator, scaling/relevance theorem, observable algebra, and universality statement
  - hinge theorem packs still missing: quadratic emergence, signature/arrow/cone coupling, MDL Lyapunov descent, constraint closure, and universality instances
  - followthrough:
    added `docs/planning/rg_toy_completion_findings_20260329.md` and synced `README.md`, `TODO.md`, `CHANGELOG.md`

- 2026-03-29 Resonance and Overlap thread:
  - source: db after async pull of online UUID `69c8913d-5240-839b-9bf8-d757ae8b208a` (title: `Resonance and Overlap`), canonical thread ID `343e73cc6a60cd1f29be15301a69aed0fa682002`
  - main finding:
    `CLOCK` should be modeled as a cyclic `Z/6` lift of `DASHI`'s `Z/3` phase, not as a dihedral construction
  - stronger schema:
    phase alone is only kinematics; admissible physical evolution requires cone admissibility, contractive / Lyapunov directionality, and MDL descent
  - tightened reading:
    the extra `CLOCK` bit is microphase / half-step refinement, not a symmetry involution, and the safe structural analogy is `CLOCK -> DASHI` as proposal/refinement -> admissible coarse phase
  - followthrough:
    added `docs/planning/resonance_and_overlap_findings_20260329.md` and synced `README.md`, `TODO.md`, `CHANGELOG.md`

- 2026-03-29 TEMP ZOS/SL bridge bundle review:
  - source: local review of `/home/c/Documents/code/ITIR-suite/TEMP_zos_sl_bridge_impl`
  - main findings:
    - retrieval over-claims manifold awareness today
    - query and shard features do not intersect enough for cosine similarity to be semantically meaningful
    - domain scoring is effectively inactive because the query domain is always unset
    - surrogate hash-derived resonance is mixed into the core score even though it is not grounded in `SL` facts or zkperf invariants
    - `pytest -q` in `TEMP_zos_sl_bridge_impl/python` fails during collection with `ModuleNotFoundError: No module named 'zos_pipeline'`
  - governance read:
    keep `SL` as truth authority, keep `ZOS` in the proposal layer only, make domain/manifold logic real before trusting it, and add an explicit admissibility / acceptance boundary before widening implementation
  - followthrough:
    added `docs/planning/temp_zos_sl_bridge_impl_review_20260329.md` and synced `README.md`, `TODO.md`, `CHANGELOG.md`

- 2026-03-29 TEMP ZOS/SL bridge bundle remediation pass:
  - source: current working turn
  - scope chosen:
    packaging/tests, query/shard feature intersection, and real query-sensitive domain scoring
  - implementation:
    - added `pythonpath = ["."]` plus a minimal setuptools build-system to `TEMP_zos_sl_bridge_impl/python/pyproject.toml`
    - changed `feature_map.py` so shard and query vectors now share lexical `lex::*` features derived from structured fact content, predicate hints, selectors, and role hints
    - changed `retrieval.py` so shard domains prefer explicit `domain_hint` values and otherwise fall back to clustered trace windows, with query domain assigned by nearest centroid instead of a constant `None`
    - left resonance weighting unchanged on purpose; it remains a later governance/admissibility follow-up
  - validation:
    - `pytest -q` in `TEMP_zos_sl_bridge_impl/python` passed (`3 passed`)
    - `python -m zos_pipeline.cli --query "tenant mould complaint"` ran successfully
  - followthrough:
    - updated `TEMP_zos_sl_bridge_impl/README.md`
    - updated `TEMP_zos_sl_bridge_impl/docs/INTEGRATION_NOTE.md`
    - updated `docs/planning/temp_zos_sl_bridge_impl_review_20260329.md`
    - synced `TODO.md` and `CHANGELOG.md`
  - remaining direction:
    the preferred operator shape is now explicit:
    `manifold_aware_rank -> candidate set -> admissibility filter -> promoted outputs`
    so the bundle stops at proposal/ranking until an acceptance boundary is implemented

- 2026-03-29 ITIR rehearsal supports real promoted manifest + tar:
  - source: current working turn
  - main decision:
    - `resolve_selector_to_container_member` and selector/tar rehearse CLI commands now allow the container fixture to be optional, inferring member paths (`shardId.cbor` or `payload/shardId.cbor`) when missing
    - harness can now point directly at `/tmp/erdfa-promoted-manifest.json` and `/tmp/erdfa-demo.tar` emitted by `cargo run --example demo` in `/home/c/Documents/code/erdfa-publish-rs`
  - validation:
    - `pytest -q tests/test_hf_container_rehearsal.py tests/test_erdfa_manifest_promotion_fixture.py`
  - followthrough:
    - recorded in `TODO.md` and `CHANGELOG.md`

- 2026-03-29 Spectral post-selector retrieval contract pass:
  - source: current working turn
  - planning note:
    `docs/planning/spectral_post_selector_retrieval_contract_20260329.md`
  - main decision:
    - spectral/eigenvector retrieval is now pinned as an optional ranking layer
      that runs only after selector resolution and before fetch
    - correct ordering is:
      selector -> logical shard ids -> spectral ranking -> fetch subset
    - the feature-basis rule is explicit:
      only structured SL facts / predicates / roles / qualifiers / zkperf
      traces or similarly modeled system outputs are allowed
    - abstention is required when manifold/domain validity fails; ranking must
      not invent certainty where no stable neighborhood exists
    - when HF batching is also present, ranking still occurs before container
      resolution
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 Shard stack layer-order summary pass:
  - source: current working turn
  - planning note:
    `docs/planning/shard_stack_layer_order_20260329.md`
  - main decision:
    - the current shortest stable stack summary is now pinned as:
      `SL -> logical shard/artifact contract -> optional spectral ranking -> optional HF container/index -> sink fetch -> Zelph`
    - this note exists as the anti-confusion reference so truth, shard
      identity, ranking, batching, and retrieval do not collapse back into one
      vague "API" layer
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 zkperf-on-SL roadmap pass:
  - source: current working turn
  - planning note:
    `docs/planning/zkperf_on_sl_roadmap_20260329.md`
  - main decision:
    - `zkperf` on `SL` is now pinned as the next gating lane before the
      previously-ranked implementation steps around HF container fixtures,
      richer `erdfa-publish-rs` manifests, and rehearsal harnesses
    - the intended role is:
      structured, receipt-bearing execution/proof material represented on the
      `SL` side before it is used by ranking or artifact-linkage layers
    - `zkperf` must remain non-authoritative for truth promotion and must not
      redefine the logical shard contract
    - the next smallest useful deliverable after this roadmap is a tiny
      `zkperf_on_sl_contract_v1` note plus fixture
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass

- 2026-03-29 zkperf-on-SL contract v1 pass:
  - source: current working turn
  - planning note:
  ...
  - main decision:
    - the first bounded `zkperf`-on-`SL` shape is now pinned as
      `ZKPerfObservation`
    - it is explicitly observational and receipt-bearing, not a truth
      promotion surface
    - required fields now cover:
      trace/run identity, provenance/source ref, structured `metrics`,
      `trace_refs`, `proof_refs`, optional `related_artifact_refs`, and a
      deterministic `hash`
    - artifact linkage is allowed only through existing artifact/shard ids,
      never via sink path semantics
  - followthrough:
    - recorded in `README.md`, `TODO.md`, and `CHANGELOG.md`
    - no code changes were made in this pass
...
- `2026-03-28` ITIR / SensibLaw PostgreSQL schema and deployment bundle:
  - added
    `docs/planning/itir_sensiblaw_postgres_schema_and_deployment_bundle_20260328.md`
    as the execution-oriented refinement of the production pack
  - current repo-facing decision:
...
- `2026-03-29` affidavit local-first proving slice decision:
  - added
  ...
  - implemented surface:
    ...
    - focused regression coverage in:
      `SensibLaw/tests/test_affidavit_coverage_review.py`
      and `SensibLaw/tests/test_query_fact_review_script.py`
...
- `2026-03-29` affidavit relation-classifier followthrough:
  - supervised a delegated bounded implementation pass inside the `SensibLaw`
    affidavit lane only
  - `SensibLaw/src/fact_intake/read_model.py` now computes typed contested
    affidavit relations with explicit subject/action/polarity-aware checks and
    emits:
    - `relation_type`
    - `relation_root`
    - `relation_leaf`
    - `explanation`
    - `missing_dimensions`
  - dominant section bucketing in the proving slice now resolves from typed
    relation output rather than only from coverage/support heuristics
  - `SensibLaw/tests/test_query_fact_review_script.py` now asserts
    `relation_type` and classification behavior on the proving-slice output
  - local validation passed:
    `cd /home/c/Documents/code/ITIR-suite && .venv/bin/python -m pytest -q SensibLaw/tests/test_query_fact_review_script.py SensibLaw/tests/test_affidavit_coverage_review.py`
    -> `32 passed`
...
- `2026-03-29` Johl Mary-parity fixture interpretation:
  - the live Johl affidavit / response pair should now be treated as the
    primary Mary-parity fixture for the next affidavit-lane quality pass
  - highest-signal finding:
    the main remaining failure is not simply substitution undercounting; it is
    weak duplicate-root and same-incident sibling-leaf handling across sides
  - next reading:
  ...
  - direct examples under review:
    - `p2-s5`, `p2-s6`, `p2-s38`, `p2-s39` look like support failures caused
      by cross-swapped sibling leaves inside shared incident roots
    - `p2-s21` still looks closer to adjacent event or substitution than
      support
  - docs and TODO now treat duplicate-root / side-local leaf reconciliation as
    the next honest priority ahead of broader substitution widening
- `2026-03-29` first duplicate-root affidavit followthrough:
  - updated `SensibLaw/scripts/build_affidavit_coverage_review.py` so the
    builder can promote a duplicate or near-duplicate support clause ahead of a
    nearby contextual clause and now emits:
    - `claim_root_id`
    - `claim_root_text`
    - `claim_root_basis`
...
- `2026-03-29` Agda-boundary docs alignment:
  - checked the Agda-facing SensibLaw docs against the newer `CLOCK` /
    `DASHI` doctrine
  - updated `SensibLaw/docs/interfaces.md` and
    `SensibLaw/docs/plan_qg_unification_sl_da51_agda_contract_20260324.md` so
    any later Agda formalization preserves the cyclic `Z/6 -> Z/3` lift
    reading, avoids dihedral phrasing, and keeps admissibility on the cone /
    contraction / MDL side instead of treating phase labels as sufficient
  - updated `SensibLaw/todo.md` so future Agda work keeps the same guardrail
- `2026-03-29` TEMP ZOS/SL bridge admissibility stage:
  - supervised a delegated lane-1 implementation pass scoped to
    `TEMP_zos_sl_bridge_impl/python`
  - the temp bridge now has an explicit post-ranking boundary:
    - `manifold_aware_rank(...)` remains candidate generation
    - `admissibility_filter(...)` emits accepted/rejected decisions
    - `manifold_aware_select(...)` composes the two
  - CLI now supports `--admit`, `--min-score`, and `--min-spectral`
  - local validation passed:
    - `cd TEMP_zos_sl_bridge_impl/python && pytest -q` -> `4 passed`
    - `python -m zos_pipeline.cli --query "tenant mould complaint" --admit`
  - current remaining gap is no longer boundary absence; it is policy tuning:
    admissibility thresholds remain provisional and resonance still sits in the
...
## 2026-04-02 legal moonshot checkpoint

- Current legal-expansion read is now explicit:
  - AU/legal surfaces should grow under the same compiler rule as AU/GWB/Nat:
    bounded evidence in -> promoted outcomes out -> derived packet/report/graph
    after that
  - case follow, authority follow, supporting-legislation/cited-instrument
    understanding, and derived legal-follow graph surfaces are the next
    high-signal AU/legal expansion points
  - graph remains derived and challengeable, not truth
- Anti-panopticon posture is now being made more explicit for moonshot/legal
  expansion:
  - existing core: `SensibLaw/docs/panopticon_refusal.md`
  - new explicit red-team note:
    `SensibLaw/docs/red_team_anti_panopticon.md`
  - legal moonshot note:
    `docs/planning/legal_moonshot_au_follow_graph_and_panopticon_boundary_20260402.md`
- Landed first bounded AU legal-expansion slice:
  - `SensibLaw/src/policy/legal_follow_graph.py`
  - `SensibLaw/src/au_semantic/semantic.py` now emits structured AU
    authority-receipt legal-ref and citation detail
  - AU fact-review bundles now emit
    `semantic_context.legal_follow_graph`
  - AU operator views now expose `legal_follow_graph.summary`
  - AU compiler contract now lists `legal_follow_graph` as a derived product
  - current graph distinguishes:
    - `case_ref`
    - `supporting_legislation`
    - `cited_instrument`
  - authority-follow operator packets now expose reference-class counts and
    structured legal-ref/citation detail to make the attachment depth
    inspectable without widening promotion logic
  - derived graph now merges richer receipt/conjecture metadata into shared
    nodes so later authority receipts do not lose provenance to earlier
    sparse event hints
  - supporting-legislation and cited-instrument nodes/edges now retain
    structured ref detail where available, and citation/authority-receipt
    nodes now retain citation plus paragraph/linkage provenance where
    available
  - attachment-bearing graph nodes now also accumulate bounded supporting
    event and supporting receipt provenance where available
  - focused validation: `21 passed`

## 2026-04-02 suite product-stack business-logic follow-through

- product-stack-first priority was reaffirmed over old domain proving grounds;
  `itir-svelte` is currently deprioritized in favor of business-logic families
- landed next producer/state hardening slice:
  - `openrecall/openrecall/normalized_artifact.py` now emits a
    producer-owned `itir.normalized.artifact.v1` `source_artifact` sidecar for
    captured screenshots
  - the same module now also emits a bounded non-authoritative
    `derived.openrecall.search_follow` artifact and a normalized
    `derived_product` wrapper for OpenRecall search results
  - focused coverage added in `openrecall/tests/test_normalized_artifact.py`
  - root governance now covers the OpenRecall artifact in
    `tests/test_cross_adopter_governance.py`
  - `StatiBaker/sb/suite_normalized_artifact.py` now guarantees the canonical
    compiled-state artifact ID remains in
    `lineage.upstream_artifact_ids` and bundle exports may pin an explicit
    `context_envelope_ref`
- validation:
  - root: `.venv/bin/python -m pytest -q tests/test_cross_adopter_governance.py tests/test_suite_normalized_artifact_schema.py` -> `4 passed`
  - OpenRecall: `../.venv/bin/python -m pytest -q tests/test_normalized_artifact.py` -> `4 passed`
  - OpenRecall `py_compile` passed for `openrecall/normalized_artifact.py`
  - `itir-svelte` was intentionally not advanced in this round
- 2026-04-03 Wikidata/Nat live-follow source-order lift:
  - `SensibLaw/src/ontology/wikidata_nat_live_follow_executor.py` now
    implements `named_query_link` by resolving locally pinned Nat packet
    query-link surfaces first and fetching the bounded target directly
  - lookup path is:
    - QID sidecar fixture first
    - packet attachment coverage index second
  - focused validation:
    - `../.venv/bin/python -m pytest -q tests/test_wikidata_nat_live_follow_executor.py tests/test_wikidata_cli.py -k 'live_follow_execute or live_follow_campaign'`
    - `7 passed`
  - live rerun:
    - `PYTHONPATH=SensibLaw .venv/bin/python -m SensibLaw.cli wikidata nat-live-follow-execute --input SensibLaw/tests/fixtures/wikidata/wikidata_nat_live_follow_campaign_20260403.json --category split_heavy_business_family --output /tmp/nat_live_follow_split_heavy_query_link_live.json`
  - result:
    - `split_heavy_business_family` now resolves both rows through
      `named_query_link`
    - `chosen_source_class_counts = {"named_query_link": 2}`
  - `SensibLaw/src/ontology/wikidata_nat_live_follow_executor.py` now also
    implements `named_reference_url` from locally pinned Nat reference surfaces
    such as:
    - packet `source_url`
    - grounding-depth `follow_receipt_url`
    - Cohort C `reference_anchor`
  - focused validation:
    - `../.venv/bin/python -m pytest -q tests/test_wikidata_nat_live_follow_executor.py tests/test_wikidata_cli.py -k 'live_follow_execute or live_follow_campaign'`
    - `9 passed`
  - live reruns:
    - policy-risk:
      `chosen_source_class_counts = {"named_reference_url": 2}`
    - reconciled non-business variance:
      still falls back to `named_revision_locked_source`
      after `named_reference_url` fetch-error due to no local concrete
